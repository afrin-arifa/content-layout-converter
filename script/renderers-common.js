function renderTable(node) {
  ...
  const lines = [];
  lines.push('<table class="table table-bordered table-striped">');
...
function renderParagraph(item, indent) {
  ...
  if (
    ...
    const buttonLink = temp.querySelector("a[href]");

    let label = "Book Appointment";
    let href = "/about-us/contact-us";

    if (buttonLink) {
      label = cleanText(buttonLink.textContent) || label;

      href = sanitizeHref(buttonLink.getAttribute("href")) || href;
    } else {
      label = text.replace(/^button:\s*/i, "").trim() || label;
    }

    return [
      `${indent}<a href="${href}" class="theme-btn mt-4" rel="nofollow">${label}</a>`,
    ];
  }
...
function buildNoImage(sections, options = {}) {
  ...
  sections.forEach((section, index) => {
    ...
    lines.push('      <div class="col-lg-10 text-center mx-auto">');
